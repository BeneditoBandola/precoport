...
st.set_page_config(
    page_title="Central de Disparo - PDV Pet", page_icon="🐾", layout="centered"
)

st.title("🚀 Central de Disparo Automatizado")
st.write(
    "Selecione a filial, o tipo de relatório desejado e clique no botão para"
    " executar o processo de forma automática."
)
...
def gerar_pdf_organizado(
    filial, df_precos_filial, df_op_filial, tipo_relatorio
):
  ...
  elements = []
  elements.append(
      Paragraph("<b>Relatório Executivo - PDV Pet</b>", title_style)
  )
  elements.append(
      Paragraph(
          f"<b>Filial:</b> {filial} | <b>Gerado em:</b>"
  ...
  if tipo_relatorio in ["Enviar Somente Preços Fora do Range", "Enviar Todos"]:
    ...
    if not df_precos_filial.empty:
      ...
      elements.append(
          Paragraph(
              "✅ Nenhum preço acima do teto detectado.",
              ParagraphStyle(
                  "OkPreco",
                  fontSize=9,
  ...
  if tipo_relatorio in ["Enviar Lista de Oportunidades", "Enviar Todos"]:
    elements.append(
        Paragraph("2. Oportunidades por Categoria (Item a Item)", sec_style)
    )
    if not df_op_filial.empty:
      ...
      elements.append(
          Paragraph(
              ...
              ParagraphStyle(
                  "OkOp", fontSize=9, textColor=colors.HexColor("#059669")
              ),
          )
      )
...
enviar_apenas_para_mim = st.checkbox(
    "Enviar apenas para o meu e-mail (Modo Teste)", value=True
)
...
tipo_relatorio_escolhido = st.radio(
    ...
    [
        "Enviar Somente Preços Fora do Range",
  ...
  if not REMETENTE_EMAIL or not REMETENTE_SENHA:
    ...
    try:
      status_container.write(
          "🌐 Conectando ao site do PDV Pet e baixando dados..."
      )
      ...
      status_container.write("📊 Processando cruzamentos e filtrando dados...")
      ...
      df["Pdv_Com_Cidade"] = df.apply(
          lambda r: (
              f"{r['Pdv']} - {r['Cidade_Clean']}/{r['Uf_Clean']}"
              if r["Cidade_Clean"] != ""
              else str(r["Pdv"])
          ),
          axis=1,
      ...
      df["Chave_Preco"] = df.apply(
          lambda r: f"{r['OpcaoEmbalagem_Clean']} -"
          f" {r['Embalagem_Clean']}".strip(" -"),
          axis=1,
      )

      df_p9 = df[
          (df["Data_Parsed"] >= INICIO_P9) & (df["Data_Parsed"] <= FIM_P9)
      ]
      df_p10 = df[
          (df["Data_Parsed"] >= INICIO_P10) & (df["Data_Parsed"] <= FIM_P10)
      ]
      ...
      df_oportunidades = pd.merge(
          p9_pares,
          p10_pares,
          on=["Distribuidor", "Cidade_Clean", "Pdv_Com_Cidade", "Item"],
          how="left",
          indicator=True,
      )
      df_oportunidades = df_oportunidades[
          df_oportunidades["_merge"] == "left_only"
      ].drop(columns=["_merge"])
      ...
      for _, row in df_p10.iterrows():
        ...
        preco = row["Preco_Num"]
        if (
            chave in PRECOS_MAXIMOS
            and pd.notnull(preco)
            and preco > PRECOS_MAXIMOS[chave]
        ):
          alertas_preco.append({
              ...
              "Item_Nome": chave,
              "Preco_Lido": preco,
              "Preco_Maximo": PRECOS_MAXIMOS[chave],
              "Status": row["Status"],
          })
        ...
        if os.path.exists(caminho_pdf):
          with open(caminho_pdf, "rb") as f:
            part = MIMEApplication(
                f.read(), Name=os.path.basename(caminho_pdf)
            )
      ...
      status_container.update(
          label="✅ Processo concluído com sucesso!", state="complete"
      )
